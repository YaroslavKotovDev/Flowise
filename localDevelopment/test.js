...
async function initialize() {
    ...
    try {
        ...
        accessToken = tokens[tokens.length - 1].accessToken
    } catch (error) {
        console.error(error)
    }
}
...
const updateEvent = async (accessToken, eventId, remindAt) => {
    ...
    const response = await fetch('https://www.zohoapis.eu/crm/v2/Events', {
        method: 'PUT',
        headers: {
...
const init = async () => {
    ...
    // const remindAt = subtractFifteenMinutesAndValidate(isoFormattedDate)
    // console.log(remindAt)
    // const endTime = addThirtyMinutesAndValidate(isoFormattedDate)
    // let participantsArray = []
    // let participant = new Participant()
    //
    // participant.setParticipant('kotov20010731@gmail.com')
    ...
    // try {
    //     await createEvent({
    //         Event_Title: `Meeting with Snow`,
    //         All_day: false,
    //         Start_DateTime: isoFormattedDate,
    ...
    // } catch (error) {
    //     console.log(error)
    //     return `It was not possible to create a scheduled call for this time and date, try another date or time`
    // }
    ...
    // const selectedRandomSlots = selectRandomSlots(freeSlots)
    // console.log(selectedRandomSlots);
    ...
    //         const secondOption = selectedRandomSlots[1] ? `date: ${selectedRandomSlots[1].date}, time: ${selectedRandomSlots[1].time}` : ''
    //         const thirdOption = selectedRandomSlots[2] ? `date: ${selectedRandomSlots[2].date}, time: ${selectedRandomSlots[2].time}` : ''
    //         return `Simply say and present like list 'All provided slots (Options: 1 - ${firstOption}, 2 - ${secondOption}, 3- ${thirdOption})
    //         That time slots is indicated in the Pacific Standard Time'`
    //     } else if (!selectedRandomSlots || !selectedRandomSlots.length) {
    //         return 'Error: Unable to schedule call. Please, contact Support'
    //     }
    ...
    //     return `Ask about another day, since this day is not a working day, write that working days are from ${days[0]} to ${days[days.length - 1]
    //         }`
    // } else if ($time !== '' && !isWithinBusinessHours($time)) {
    //     return `Ask about another time, since this time is not working time, write that working time is ${startTime.time + ' ' + startTime.period
    ...
    //         const response = await updateEvent(accessToken, recordId, '15 mins')
...
init()
    ...
    .catch((err) => {
        console.log(err)
        return 'Error: Unable to schedule call. Please, contact Support ' + err
    })
